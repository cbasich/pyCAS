...
class PathDomainModel():
    def __init__(self, map_info, gamma=1.0):
        self.gamma = gamma
    ...
    def generate_actions(self):
        ...
        for edge in self.edges.values():
            actions.append((edge['s0_id'], edge['s1_id'], edge['max_speed']))
        return actions
        ...
        for s, state in enumerate(self.states):
            ...
            for a, action in enumerate(self.actions):
                if state[0] != action[0]:
                    ...
                    statePrime = (action[1], self.nodes[action[0]]['loc']['x'], self.nodex[action[0]]['loc']['y'])
