...
class CampusDeliveryBotHelper():
    ...
    def extract_state_features(self, state):
        ...
        features = [feature + "_" + self.get_state_feature_value(state, feature) for feature in used_features if self.get_state_feature_value(state, feature) != None]
        if "obstacle_crosswalk" in features:
            features[features.index('crosswalk')] = "obstcle_" + str(state[5])
        return features
