...
class CAS():
            ...
            for a, action in enumerate(self.DM.actions):
                ...
                for level_index in np.argsort(-1.0 * np.array([self.potential[s][a][l] for l in L])):
                    if np.random.uniform() <= self.potential[s][a][L[level_index]]:
                        if L[level_index] == 3 and len(state) > 4:
                            if ((state[5] == 'door-closed' and action[0] == 'open'
                                and (self.DM.helper.get_state_feature_value(state, 'door_type') == 'pull'
                                  or self.DM.helper.get_state_feature_value(state, 'door_size') == 'heavy'
                                 or (self.DM.helper.get_state_feature_value(state, 'door_size') == 'medium'
                                 and self.DM.helper.get_state_feature_value(state, 'region') == 'b2')))
                            or (action[0] == 'cross' and (state[3] == 'rainy' or state[3] == 'clear' and
                                  ((state[2] == 'daytime' and self.DM.get_state_feature_value(state, 'visibility') == 'low' and state[5] == 'traffic_heavy')
                                or (state[2] == 'nighttime' and self.DM.get_state_feature_value(state, 'visibility') == 'high' and state[5] == 'traffic_heavy')
                                or (state[2] == 'nighttime' and self.DM.get_state_feature_value(state, 'visibility') == 'low' and state[5] != 'traffic_none'))))
                            or self.HM.lambda_[state][action][2] < 0.9):
                                break

                        if L[level_index] == 0 and len(state) > 4:
                            if ((state[5] == 'door-closed' and action == 'open' and self.DM.helper.get_state_feature_value(state, 'door_type') == 'push'
                                and (self.DM.helper.get_state_feature_value(state, 'door_size') == 'light'
                                 or (self.DM.helper.get_state_feature_value(state, 'door_size') == 'medium'
                                 and self.DM.helper.get_state_feature_value(state, 'region') != 'b2')))
                            or (action == 'cross' and state[3] == 'clear' and ((state[2] == 'daytime' and self.DM.helper.get_state_feature_value(state, 'visibillity') == 'high')
                                or (state[2] == 'daytime' and self.DM.helper.get_state_feature_value(state, 'visibillity') == 'low') and (state[5] != 'traffic_heavy')
                                or (state[2] == 'nighttime' and self.DM.helper.get_state_feature_value(state, 'visibillity') == 'high') and (state[5] != 'traffic_heavy')
                                or (state[2] == 'nighttime' and self.DM.helper.get_state_feature_value(state, 'visibillity') == 'low') and (state[5] == 'traffic_none')))
    ...
    def check_level_optimality(self):
        ...
        total, correct = 0., 0.
        for s, state in enumerate(self.states):
            if len(state[0]) < 5 or 'open' in state[0][5]:
                continue
            total += 1
            action = self.pi[s]
