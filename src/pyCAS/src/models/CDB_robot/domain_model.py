...
class DeliveryBotDomain():
    def __init__(self, filename, start, destination, gamma=1.0):
        ...
        with open(os.path.join(MAP_PATH, 'obstacle_info.json')) as F:
            self.obstacle_info = json.load(F)
    ...
    def generate_states(self, start, destination):
        ...
        for s in S:
            ...
            if self.grid[x][y] == 'D':
                ...
                if start == self.grid[x][y] or start == (x, y):
                    self.init = s
                elif destination == self.grid[x][y] or destination == (x, y):
                    goals.add(s)
                states.add(s)
                if s not in self.kappa.keys():
                    self.kappa[s] = {}
